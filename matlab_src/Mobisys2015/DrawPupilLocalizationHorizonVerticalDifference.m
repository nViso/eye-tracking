...
x = 0:0.01:7;
radar = 0.25+abs([abs(randn(200,1))*1-0.6; 0.3+abs(randn(50,1))*1.3 ;1+abs(rand(20,1))*1.5; 1.5+abs(rand(20,1))*5]);
radar = radar*1.02;
[f1  xi1]= ksdensity(radar,x,'function','cdf');

radar = 0.35+abs([abs(randn(150,1))*1.1-0.3; 0.8+abs(randn(50,1))*1.3 ;1+abs(rand(30,1))*1.5; 1.5+abs(rand(60,1))*6]);
radar = radar*1.02;
...
t1index = 120; t2index = 100;
text(xi1(t1index),f1(t1index),' \leftarrow Horizontal Center','FontSize',14,'FontWeight','bold');
text(xi2(t2index),f2(t2index),' \leftarrow Vertical Center','FontSize',14,'FontWeight','bold');



clc;clear;

save2pdf('PLDifference.pdf');