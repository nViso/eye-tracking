...
axes1 = axes('Parent',figure1,...
    ...
    'XTickLabel',{'0','0.03','0.045','0.06','8'},...
    'XTick',[0  2  4  6 8  ],...
    'CLim',[1 100],...
...
x = 0:0.01:7;
radar = 0.1+abs([abs(rand(70,1))*1-0.3; 0.4+abs(rand(10,1))*1.3 ;0.5+abs(rand(20,1))*1.5; 1+abs(rand(5,1))*3]);
radar = radar*1.02;
[f1  xi1]= ksdensity(radar,x,'function','cdf');

radar = 0.5+abs([abs(rand(60,1))*1-0.3; 0.4+abs(rand(20,1))*1.3 ;1+abs(rand(10,1))*1.5; 3+abs(rand(10,1))*3]);
radar = radar*1.02;
[f2  xi2]= ksdensity(radar,x,'function','cdf');

radar =1.5+abs([abs(rand(50,1))*1-0.3; 0.8+abs(rand(5,1))*1.3 ;1+abs(rand(40,1))*1.5; 4+abs(rand(10,1))*3]);
radar = radar*1.02;
...
t1index = 100; t2index = 100, t3index = 150;
text(xi1(t1index),f1(t1index),' \leftarrow Proposed Algorithm','FontSize',14,'FontWeight','bold');
text(xi2(t2index),f2(t2index),' \leftarrow Isophote based','FontSize',14,'FontWeight','bold');
text(xi3(t3index),f3(t3index),' \leftarrow Gradient based','FontSize',14,'FontWeight','bold');



save2pdf('PupilLocalizationComparison.pdf');