...
int main (int argc, char *argv[])
{
    ...
    string input;
    cout<<"type your camera name( if this named folder exists in the camera_calibration folder, the video capture step will be skipped):";
    cin >> input;
    string cameraname  = input;
    fs::path imagesDir = cameraBaseDir / input;
    if (fs::exists(imagesDir)) {
        cameraFileExists = true;
    } else {
    ...
    vector<fs::path> images = listFilesWithExtension(imagesDir, "", "jpg");
